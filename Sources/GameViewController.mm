...
- (void)viewDidLoad { [super viewDidLoad]; self.queue=[self.device newCommandQueue]; self.status=[[UILabel alloc]initWithFrame:CGRectMake(18,18,700,40)];self.status.textColor=UIColor.whiteColor;self.status.text=@"CS2 iOS — Import cs_office n0.glb";[self.view addSubview:self.status]; UIButton*b=[UIButton buttonWithType:UIButtonTypeSystem];b.frame=CGRectMake(18,65,210,44);[b setTitle:@"Import CS2 Map" forState:UIControlStateNormal];[b addTarget:self action:@selector(importMap) forControlEvents:UIControlEventTouchUpInside];[self.view addSubview:b]; [self loadPersistedMapIfPresent]; [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(controller:) name:GCControllerDidConnectNotification object:nil];[GCController startWirelessControllerDiscoveryWithCompletionHandler:^{}]; }
...
- (void)loadPersistedMapIfPresent { NSURL*u=[self storedMapURL]; if([[NSFileManager defaultManager] fileExistsAtPath:u.path]) { self.status.text=@"Loading saved cs_office…"; [self loadMapURL:u]; } }
